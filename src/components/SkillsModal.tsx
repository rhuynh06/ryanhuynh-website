  ...
  return (
    <Dialog.Root open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <Dialog.Portal>
        ...
        <Dialog.Content
          className="fixed top-1/2 left-1/2 w-[1152px] h-[768px] -translate-x-1/2 -translate-y-1/2 
                     bg-no-repeat bg-center bg-cover rounded-lg shadow-lg overflow-hidden"
          style={{ backgroundImage: "url('/tech/main.png')" }}
        >
          {/* Scrollable Skills Panel */}
          <div className="absolute top-[120px] right-[80px] w-[480px] h-[580px] overflow-y-auto p-4 text-black font-mono">
            <ul className="grid grid-cols-5 gap-6 px-0 mx-0">
                {allSkills.map((skill) => {
